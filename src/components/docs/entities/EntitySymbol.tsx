...
type EntityProps = React.PropsWithChildren<{
  type: 'class' | 'function' | 'interface' | 'property' | 'variable' | 'enum';
  id?: string;
  link?: boolean;
  source?: string;
}>;

export function EntitySymbol({
  type,
  children,
  id,
  link,
    ...
    case 'variable':
      c = (
        <div
          ...
          onClick={updateLink}
        >
          <VscSymbolVariable className="h-6 w-6 text-blue-600" />
          <Heading.H4 id={id}>{children}</Heading.H4>
        </div>
      );
      break;
    case 'enum':
      c = (
        <div
          ...
          onClick={updateLink}
        >
          <VscSymbolEnum className="h-6 w-6 text-blue-600" />
          <Heading.H4 id={id}>{children}</Heading.H4>
        </div>
      );
      break;
